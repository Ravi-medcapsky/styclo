...
function ProductPage({ onAddToCart }) {
  ...
  const handleAddToCartClick = () => {
    alert(`Added ${isadded} x ${product.title} (Size: ${selectedSize}) to your cart!`);
    onAddToCart(product, isadded, selectedSize); 
    navigate("/cart");
  };
  
  const handleBuyNow = () => {
    alert(`Proceeding to checkout with ${isadded} x ${product.title} (Size: ${selectedSize}).`);
    navigate("/checkout");
  };
    ...
    <div className="flex flex-col  justify-between gap-10 p-10">
      <div className="flex flex-col md:flex-row justify-between gap-10 p-10">
        ...
        <div className="flex flex-col  gap-1 ">
          <h2 className="font-semibold text-4xl self-center mb-5">{product.title}</h2>
          <p className="text-xl mb-5 pl-5">{product.description}</p>
